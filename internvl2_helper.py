...
class OvModelForCausalLMWithEmb(GenerationMixin):
    def __init__(self, model_dir, device="CPU", ov_config=None, compile=True) -> None:
        ...
        self.config = AutoConfig.from_pretrained(model_dir, trust_remote_code=True).llm_config
...
class OVInternVLChatModel:
    ...
    def __init__(self, model_dir: Path, device: str):
        ...
        self.language_model = OvModelForCausalLMWithEmb(model_dir, device)
...
# response = ov_model.chat(tokenizer, pixel_values, question, generation_config)
